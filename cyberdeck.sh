...
# Siderail & Branding
readonly c_rail="\033[38;5;39m"      # VGT Neon Cyan
readonly c_brand="\033[38;5;81m"     # Light Cyan
readonly c_label="\033[38;5;244m"    # Mid Gray
readonly c_val="\033[38;5;255m"      # Pure White

# Status Colors
readonly c_ok="\033[38;5;113m"       # Poison Green
readonly c_warn="\033[38;5;220m"     # Hazard Yellow
readonly c_crit="\033[38;5;196m"     # Blood Red
readonly c_mag="\033[38;5;170m"      # Cyber Magenta
readonly c_bar_bg="\033[38;5;235m"   # Void Black

# VGT Semantic Glyphs
readonly rail_char="▊"               # Massive Hardware Siderail
readonly g_arr="▶"
readonly g_dot="■"
...
LOAD1_INT=${LOAD1%%.*} # Integer für Threat-Calc (schneidet Kommastellen sicher ab)
...
# Top Process (O(1) CPU Limit)
read -r TOP_CMD TOP_CPU < <(ps -eo comm,pcpu --sort=-pcpu | awk 'NR==2{print $1, $2}')

# Network Interface & Traffic (Robust Regex für Interface Erkennung)
...
check_service() {
    if systemctl is-active --quiet "$1" 2>/dev/null || ( [ -x "/etc/init.d/$1" ] && "/etc/init.d/$1" status 2>/dev/null | grep "running" >/dev/null ); then
...
if [ -d "/www/server/panel" ] || systemctl is-active --quiet bt 2>/dev/null; then CP_NAME="aaPanel"; CP_SVC="bt"
elif [ -d "/usr/local/psa" ] || systemctl is-active --quiet psa 2>/dev/null || systemctl is-active --quiet sw-cp-server 2>/dev/null; then CP_NAME="Plesk"; CP_SVC="psa"; systemctl is-active --quiet sw-cp-server 2>/dev/null && CP_SVC="sw-cp-server"
elif [ -d "/usr/local/cpanel" ] || systemctl is-active --quiet cpanel 2>/dev/null; then CP_NAME="cPanel"; CP_SVC="cpanel"
fi
[ -n "$CP_SVC" ] && SVC_CP=$(check_service "$CP_SVC" "$CP_NAME") || SVC_CP="${c_dim}[○] NoPanel${c_reset}"

# Fail2ban Extract
BANNED_COUNT=0
[ -r /var/log/fail2ban.log ] && BANNED_COUNT=$(tail -n 10000 /var/log/fail2ban.log | grep -c "Ban " || echo 0)

# THREAT LEVEL CALCULATOR (Dynamic Intelligence)
...
# --- SEKTION 1: THREAT INTEL (Sicherheit zuerst) ---
...
last -a -n 3 2>/dev/null | while read -r line; do
